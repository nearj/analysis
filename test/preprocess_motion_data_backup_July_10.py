...
EXT_JSON = '.json'
EXT_EXCEL = '.xlsx'
SAVE_DIR_PD = './data/preprocessed/motion/prob_dist/'
SAVE_DIR_CD = './data/preprocessed/motion/cumul_dist/'
...
def raw_to_prob_densities(raw_data_set):
    ...
    ret = []
    # for raw_data in raw_data_set: # raw_data 180
    for raw_data in raw_data_set: # raw_data 180
        # for frame in raw_data:
        raw_data = raw_data[:60]
        u = np.unique(raw_data, axis = 0, return_counts=True)
        # print(len(raw_data))
        u = (u[0], u[1] / u[1].sum())
        # print(u[1])
        tmp = []
...
def process():
    ...
    return raw_to_prob_densities(raw_data_set), raw_meta_set #, raw_to_cumul_densities(raw_data_set), raw_meta_set
...
def main(args):
    ...
    data_set = list(zip(*([data_set[0], data_set[1]])))
