...
nfrom matplotlib import pyplot as plt
...
def load_motion_data_dir(directory = LOAD_MOTION_DIR):
    ...
    Args:
       directory (str) directory to load data (default LOAD_MOTION_DIR =
           './data/preprocessed/motion/')

    Returns:
        {(str: file name): (list: list of data about 180 frames)} 
    """
    ret = {}
...
def load_video_data_from_dir(directory = LOAD_VIDEO_DIR):
    """ load video data from directory
    
    Args:
       directory (str) directory to load data(default LOAD_VIDEO_DIR =
           'data/preprocess_video_data')
...
def save_to_dir(data_list, action, directory):
    """ save data to directory with some action
